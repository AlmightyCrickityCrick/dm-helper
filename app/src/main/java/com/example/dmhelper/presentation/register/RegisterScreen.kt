...
import com.example.booksharing.presentation.authscreen.register.RegisterViewModel
import com.example.booksharing.presentation.common.AuthEvent
import com.example.booksharing.presentation.components.button.PrimaryButton
import com.example.booksharing.presentation.components.input.PasswordInput
...
private fun RegisterResult(
    ...
    val defaultError = stringResource(R.string.error_register_network)
    LaunchedEffect(key1 = Unit) {
        lifecycle.repeatOnLifecycle(state = Lifecycle.State.STARTED) {
            launch {
                eventChannel.collectLatest { event ->
                    handleEvents(event, onRegisterSuccess, snackbarState, defaultError)
                }
            }
        }
    }
